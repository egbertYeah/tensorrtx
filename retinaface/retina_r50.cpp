...
IActivationLayer* bottleneck(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, \
...
ILayer* conv_bn_relu(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, \
                    ITensor& input, int outch, int kernelsize, int stride, int padding, bool userelu, std::string lname) 
{
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) 
{
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../Resnet50_Final_all.wts");
    ...
    // up3->setNbGroups(256);
    // ****** UpSample ************** // 
    IResizeLayer* up3 = network->addResize(*output3->getOutput(0));
    int num_scales = 3;
    const float scales[num_scales] = { 1, 2, 2};
    up3->setScales(scales, num_scales);
    up3->setResizeMode(ResizeMode::kLINEAR);
    ...
    // up2->setNbGroups(256);
    // ********* UpSample ********** //
    IResizeLayer* up2 = network->addResize(*output2->getOutput(0));
    up2->setScales(scales, num_scales);
    up2->setResizeMode(ResizeMode::kLINEAR);
